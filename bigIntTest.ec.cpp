...
int main() {
    ...
    LOG_MSG("Testing the * operator...",
            test_math_oper(zero, zero, Multiply()) &&
            test_math_oper(afew, zero, Multiply()) &&
            test_math_oper(zero, some, Multiply()) &&
            test_math_oper(many, alot, Multiply()) &&
            test_math_oper(number(rand() % 10000), number(rand() % 10000), Multiply())
            );

    LOG_MSG("Testing the / operator...",
            test_math_oper(zero, afew, Divide()) &&
            test_math_oper(afew, some, Divide()) &&
            test_math_oper(some, afew, Divide()) &&
            test_math_oper(some, some, Divide()) &&
            test_math_oper(alot, many, Divide()) &&
            test_math_oper(number(rand() % 10000 + 1000), number(rand() % 1000), Divide())
            );
